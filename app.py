...
# Pollinations.ai 免費圖片生成服務 (無需 API Key)
...
def generate_image_with_pollinations(scene_desc: str, character: str, scene: str) -> bytes | None:
    ...
    image_url = f"{POLLINATIONS_API}{prompt}"
    try:
        resp = requests.get(image_url, timeout=60) # Pollinations.ai 通常較快，縮短 timeout
        if resp.status_code == 200 and resp.headers.get("content-type", "").startswith("image"):
            return resp.content
        else:
            st.warning(f"插圖生成失敗（HTTP {resp.status_code}）。原始錯誤：{resp.text[:200]}")
        return None
    except requests.exceptions.Timeout:
        st.warning("⏳ Pollinations.ai 圖片生成請求超時。請稍後再試。")
        return None
...
if st.session_state.page == "generator":
    ...
    with col_r:
        st.subheader("💡 使用提示")
        st.info(
            "輸入主角、場景與主題後，點擊「開始生成」，故事與插圖將同步呈現！\n\n"
            "完成後可匯出為 PDF 或前往家長儀表板查看統計。"
        )
