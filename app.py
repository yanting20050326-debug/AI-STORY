...
pasted_content.txt
文字 · 24.66 KB
我現在在製作一個網頁 AI 繪本故事生成器
但在程式碼上遇到問題
fpdf.errors.FPDFUnicodeEncodingException: Character "皇" at index 0 in text is outside the range of characters supported by the font used: "helvetica". Please consider using a Unicode font.
Traceback:
File "/opt/render/project/src/.venv/lib/python3.14/site-packages/streamlit/runtime/scriptrunner/exec_code.py", line 129, in exec_func_with_error_handling
    result = func()
File "/opt/render/project/src/.venv/lib/python3.14/site-packages/streamlit/runtime/scriptrunner/script_runner.py", line 789, in code_to_exec
    exec(code, module.__dict__)  # noqa: S102
    ~~~~^^^^^^^^^^^^^^^^^^^^^^^
File "/opt/render/project/src/app.py", line 518, in <module>
    story_pdf = create_story_pdf(
        st.session_state.story_text,
        meta.get("character", "故事"),
        st.session_state.image_bytes,
    )
File "/opt/render/project/src/app.py", line 294, in create_story_pdf
    pdf.cell(0, 16, f"{character} 的專屬故事", ln=True, align="C")
    ~~~~~~~~^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
File "/opt/render/project/src/.venv/lib/python3.14/site-packages/fpdf/fpdf.py", line 281, in wrapper
    return fn(*args, **kwargs)
File "/opt/render/project/src/.venv/lib/python3.14/site-packages/fpdf/deprecation.py", line 36, in wrapper
    return fn(*args, **kwargs)
File "/opt/render/project/src/.venv/lib/python3.14/site-packages/fpdf/fpdf.py", line 3910, in cell
    text = self.normalize_text(text)
File "/opt/render/project/src/.venv/lib/python3.14/site-packages/fpdf/fpdf.py", line 5683, in normalize_text
...
故事生成的出來了，但插圖一直無法生成
Lite
收到，我已經看到你遇到的 FPDFUnicodeEncodingException 錯誤以及插圖無法生成的描述。我會先分析你提供的程式碼，然後幫你修復中文字體支援問題並找出插圖生成失敗的原因。請稍等。




Lite
我已經修復了 FPDFUnicodeEncodingException 錯誤，並對插圖生成部分進行了調整。主要修改如下：
FPDF 中文字體支援：
我下載了 NotoSansTC-Regular.ttf 字體檔案到專案目錄。
修改了 create_pdf_report 和 create_story_pdf 函數，讓它們嘗試載入這個字體來支援中文顯示。如果字體載入失敗，會顯示錯誤訊息。
插圖生成 API 端點調整：
我將 Hugging Face 的圖片生成 API 端點從 router.huggingface.co 暫時改回 api-inference.huggingface.co，以便測試。如果這個端點仍然無法生成圖片，我們可能需要進一步研究 FLUX.1-schnell 模型在 router.huggingface.co 上的具體使用方式和參數。
在 create_story_pdf 函數中，如果嵌入插圖失敗，現在會顯示一個警告訊息，以便追蹤問題。
請使用這個更新後的 app.py 檔案替換你現有的檔案，然後再次運行你的應用程式。如果插圖生成仍然有問題，請提供更多錯誤訊息，我會再協助你調查。
app.py
程式碼 · 24.55 KB
查看此任務中的所有檔案
任務已完成
...
def create_story_pdf(text: str, character: str, image_bytes_list: list) -> bytes:
    ...
    if image_bytes_list:
        try:
            ...
            st.warning(f"PDF 嵌入插圖失敗：{e}") # Added warning for image embedding failure
